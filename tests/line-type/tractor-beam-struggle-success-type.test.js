...
describe('tractor beam struggle line type', () => {
  test("has correct tags", () => {
    expect(TractorBeamStruggleSuccessType.getTags()).toEqual(expect.arrayContaining([LineTag.tractorBeam]));
  });
  test("detects German entry log line positively", () => {
    const testLogEntry = { "lang": "de", "entry": String.raw`[Scout] Deudi konnte sich vom Traktorstrahl losreißen.` };
    
    expect(TractorBeamStruggleSuccessType.detect(testLogEntry.entry, testLogEntry.lang)).toBe(true);
  });

  test("parses German entry log line correctly", () => {
    const testLogEntry = { "lang": "de", "entry": "[Scout] Deudi konnte sich vom Traktorstrahl losreißen." };
    const parseResult = TractorBeamStruggleSuccessType.parse(testLogEntry.entry, testLogEntry.lang);
